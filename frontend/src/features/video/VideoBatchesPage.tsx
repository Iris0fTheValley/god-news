...
export function VideoBatchesPage() {
  ...
  const selectedId = searchParams.get('batch');
  const setSelectedId = (batchId: string | null) => {
  ...
  const doneStoriesQuery = useQuery({
    queryKey: queryKeys.stories('DONE'),
    queryFn: () => listStories('DONE'),
    enabled: showCreate,
  });
  ...
  const selectedStoryIds = createForm.story_ids ?? [];
  ...
  return (
    <div className="page video-page">
      ...
      {selectedId === null ? null : <ModalDialog open className="create-drawer wide-drawer" labelledBy="video-batch-detail-heading" onClose={() => setSelectedId(null)}><div className="panel-header"><div><p className="eyebrow">BATCH DETAIL</p><h2 id="video-batch-detail-heading">批次详情</h2></div><button className="icon-button" type="button" onClick={() => setSelectedId(null)} aria-label="关闭">✕</button></div><div className="panel-body">{detailQuery.isLoading ? <div className="loading-state">正在加载批次详情…</div> : detailQuery.error !== null ? <ApiErrorNotice error={detailQuery.error} onRetry={() => void detailQuery.refetch()} /> : detailQuery.data === undefined ? null : <BatchDetail key={`${detailQuery.data.batch_id ?? 'batch'}-${String(detailQuery.data.version)}`} batch={detailQuery.data} roles={rolesQuery.data} isMutating={narrationReviewMutation.isPending || synthesizeMutation.isPending || timelineMutation.isPending || renderMutation.isPending || cancelMutation.isPending || deleteMutation.isPending} onDelete={setDeleteTarget} onCancel={(batchId) => cancelMutation.mutate(batchId)} onNarrationReview={(batch, decision, revisedScript, note, reviewerId) => narrationReviewMutation.mutate({batch, decision, revisedScript, note, reviewerId})} onSynthesize={(batch) => synthesizeMutation.mutate(batch)} onTimelineReview={(batch, decision, note, reviewerId) => timelineMutation.mutate({batch, decision, note, reviewerId})} onRender={(batch) => renderMutation.mutate(batch)} />}{mutationError === null ? null : <ApiErrorNotice error={mutationError} />}</div></ModalDialog>}

      {showCreate ? <ModalDialog open className="create-drawer" labelledBy="new-video-batch-heading" onClose={() => setShowCreate(false)}><div className="panel-header"><div><p className="eyebrow">NEW BATCH</p><h2 id="new-video-batch-heading">新建视频批次</h2></div><button className="icon-button" type="button" onClick={() => setShowCreate(false)} aria-label="关闭">✕</button></div><form className="panel-body form-grid" onSubmit={(event) => { event.preventDefault(); createMutation.mutate(createForm); }}><label className="field wide"><span>标题</span><input className="input" required value={createForm.title} onChange={(event) => setCreateForm({...createForm, title: event.target.value})} /></label><label className="field wide"><span>副标题（可选）</span><input className="input" value={createForm.subtitle ?? ''} onChange={(event) => setCreateForm({...createForm, subtitle: event.target.value === '' ? null : event.target.value})} /></label><fieldset className="field wide"><legend>选择已终审故事</legend><p className="field-hint">显式选择可保证节目只使用本次核对过的真实故事；留空时才按数量自动选择。</p>{doneStoriesQuery.isLoading ? <p className="pending-note">正在读取可用故事…</p> : doneStoriesQuery.error !== null ? <ApiErrorNotice error={doneStoriesQuery.error} /> : <div className="review-form" style={{maxHeight: 260, overflowY: 'auto'}}>{doneStoriesQuery.data?.map((story) => { const storyId = story.story_id; if (storyId === undefined) return null; const selected = selectedStoryIds.includes(storyId); return <label className="checkbox-field" key={storyId}><input type="checkbox" checked={selected} onChange={(event) => { const storyIds = event.target.checked ? [...selectedStoryIds, storyId] : selectedStoryIds.filter((candidate) => candidate !== storyId); setCreateForm({...createForm, story_ids: storyIds, max_stories: storyIds.length > 0 ? storyIds.length : createForm.max_stories}); }} /><span><strong>{story.title ?? '未命名故事'}</strong><small className="metadata">{storyId}</small></span></label>; })}</div>}</fieldset><label className="field"><span>留空选择时的故事数</span><input className="input" type="number" min={1} max={15} required value={Number.isFinite(createForm.max_stories) ? createForm.max_stories : ''} disabled={selectedStoryIds.length > 0} onChange={(event) => setCreateForm({...createForm, max_stories: event.target.valueAsNumber})} onBlur={() => { if (!Number.isFinite(createForm.max_stories)) setCreateForm({...createForm, max_stories: 5}); }} /></label><label className="field"><span>BGM（可选）</span><select className="select" value={createForm.bgm_track_id ?? ''} onChange={(event) => setCreateForm({...createForm, bgm_track_id: event.target.value === '' ? null : event.target.value})}><option value="">不使用 BGM</option>{bgmQuery.data?.map((track) => <option key={track.track_id} value={track.track_id}>{track.display_name}</option>)}</select></label><label className="field"><span>BGM 音量</span><input className="input" type="number" min={0} max={1} step={0.01} required value={Number.isFinite(createForm.bgm_volume) ? createForm.bgm_volume : ''} onChange={(event) => setCreateForm({...createForm, bgm_volume: event.target.valueAsNumber})} onBlur={() => { if (!Number.isFinite(createForm.bgm_volume)) setCreateForm({...createForm, bgm_volume: 0.12}); }} /></label><label className="checkbox-field"><input type="checkbox" checked={createForm.bgm_loop} onChange={(event) => setCreateForm({...createForm, bgm_loop: event.target.checked})} /><span>BGM 循环</span></label>{createMutation.error === null ? null : <div className="wide"><ApiErrorNotice error={createMutation.error} /></div>}<div className="form-actions wide"><button className="button" type="button" onClick={() => setShowCreate(false)}>取消</button><button className="button primary" type="submit" disabled={createMutation.isPending || (doneStoriesQuery.data?.length ?? 0) === 0}>{createMutation.isPending ? '创建中…' : '创建并审阅口播'}</button></div></form></ModalDialog> : null}
