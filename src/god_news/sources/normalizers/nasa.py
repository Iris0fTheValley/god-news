...
class NasaSourceNormalizer:
    source: Literal["nasa"] = "nasa"
    raw_type: type[RawNasaItem] = RawNasaItem

    def normalize(self, item: RawNasaItem, /) -> NormalizedSourceItem:
        ...
        return NormalizedSourceItem(
            source=self.source,
            external_id=stable_external_id(self.source, item.article_id),
            ...
            source_fields=NasaSourceFields(
                article_id=normalize_text(item.article_id, preserve_paragraphs=False),
                categories=normalize_tags(item.categories),
            ),
        )

    @staticmethod
    def _extract_media(asset: RawNasaImage | RawNasaVideo) -> MediaAsset:
        if isinstance(asset, RawNasaImage):
